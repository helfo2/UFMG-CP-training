...
public:
	...
	void calcula_menor_caminho(int partida, int destino, int rota);
};
...
void grafo::calcula_menor_caminho(int partida, int destino, int rota) {
	priority_queue<ii, vector<ii >, greater<ii > > lista_distancias;

	vi distancia(N*(N-1), INF);
	...
	while(!lista_distancias.empty()) {
		...
		for(i = adj[v].begin(); i != adj[v].end(); ++i) {
			//if((*i).first >= rota || (v < rota && (v == (*i).first + 1 || (*i).first == v + 1)) ) {
			//if((*i).first <= rota) {
				int temp = (*i).first;
				int peso = (*i).second;
...
void adiciona_estradas(grafo& cidade, int m, int c) {
	int u, v, p;

	while(m-- > 0) {
		sc3(u, v, p);

		if(u >= c || ((v < c) && (v == u + 1 || v == u - 1)) ) {
			cidade.adiciona_aresta(u, v, p);
		}
	}
}
...
int main(void) {
	...
	while(sc4(n, m, c, k), n > 0) {
		grafo cidade(n);

		adiciona_estradas(cidade, m, c);

		cidade.calcula_menor_caminho(k, c - 1, c);
	}
